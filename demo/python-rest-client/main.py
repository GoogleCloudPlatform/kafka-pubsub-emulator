...
MESSAGE_IDS = ('messageIds', 'message_ids')
RECEIVED_MESSAGES = ('receivedMessages', 'received_messages')
ACK_ID = ('ackId', 'ack_id')
...
def _publish(pubsub, project, topic):
  ...
  while True:
    ...
    message_id = response.get(MESSAGE_IDS[0], response.get(MESSAGE_IDS[1]))
...
def _pull(pubsub, project, subscription):
  ...
  while True:
    ...
    acks = []
    received_messages = response.get(RECEIVED_MESSAGES[0],
                                     response.get(RECEIVED_MESSAGES[1]))
    if received_messages:
      for m in received_messages:
        logger.info('Received {}'.format(m.get('message')))
        ack_id = m.get(ACK_ID[0], m.get(ACK_ID[1]))
        acks.append(ack_id)
